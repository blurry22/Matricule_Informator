  ...
  .get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const apiUrl =
          `https://api.mister-auto.com/v1/vehicle/global-plates/v2/ES/${matricula}`;
  ...
  .get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const response = await axios.get(apiUrl, { headers });
        const apiData = response.data;
        const data = `${apiData.make} ${apiData.model} ${apiData.version}`
  ...
  .get(
    ...
    async (req: Request, res: Response) => {
      try {
        ...
        const response = await axios.get(apiUrl, { headers });console.log(response)
        const apiData = response.data;
        const data = apiData.max_net_power_hp
        res.status(200).json(data);
      } catch (e) {
        res.status(500).send(e.message);
      }
